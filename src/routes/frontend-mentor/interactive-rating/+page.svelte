...
<section class="bg-(--very-dark-blue) grow text-white font-['Overpass'] flex flex-col px-6">
  <div class="max-w-104 min-h-104 mx-auto my-auto box px-6 pt-6 pb-8 md:p-8 rounded-[1.875rem] grid">
    <!-- Rating state start -->
    {#if !form?.success}
          ...
          <fieldset class="flex justify-between">
            {#each {length: 5} as _, i }
              <div>
                ...
                <label class="flex items-center justify-center w-10.5 h-10.5 md:w-12.75 md:h-12.75 bg-(--dark-blue) rounded-full text-(--medium-gray) peer-checked:bg-(--medium-gray) peer-checked:text-white hover:bg-(--orange) hover:text-white hover:cursor-pointer text-sm md:text-base" for="rating-{i+1}">{i+1}</label>
              </div>
            {/each}
          </fieldset>
